...
def weighted_pca(coords, weights, eps=1e-8):
    """
    coords: (N, 3) numpy array
    weights: (N,) numpy array
    """
    w = weights / (np.sum(weights) + eps)
    mean = np.sum(coords * w[:, None], axis=0)
    ...
    principal_axis = eigvecs[:, 0]  # 第一主轴
    return principal_axis, eigvals
...
def process_replicate(xtc, topo, qm_data, 
    ...
    for ts in u.trajectory:
        ...
        # 【新增】ELF 主轴 + 各向异性指标（不影响原有 orientation_feat）
        elf_axis_angle = 0.0
        elf_anisotropy = 0.0

        # 只使用 ELF 权重显著的芳香碳，避免噪声
        elf_mask = safe_w > 0.05 * np.max(safe_w)

        if np.sum(elf_mask) >= 2:
            elf_coords = lp_ring[elf_mask]
            elf_weights = safe_w[elf_mask]

            # 加权 PCA（你已经定义了 weighted_pca）
            elf_axis, elf_eigvals = weighted_pca(elf_coords, elf_weights)

            if elf_axis is not None and h6_ref_atom_start is not None and h6_ref_atom_end is not None:
                # Helix 6 参考轴
                ...
                if ref_norm > 1e-6:
                    ref_vec_u = ref_vec / ref_norm
                    elf_axis_u = elf_axis / (np.linalg.norm(elf_axis) + 1e-8)

                    # ELF 主轴 vs H6 轴夹角（cos）
                    elf_axis_angle = np.abs(np.dot(elf_axis_u, ref_vec_u))

            # 各向异性：是否存在“明确方向”
            # λ1 >> λ2 → 强方向性（Dopa / ARI）
            elf_anisotropy = (elf_eigvals[0] - elf_eigvals[1]) / (elf_eigvals[0] + elf_eigvals[1] + 1e-8)
            
            if rep_name == "gromacs_replicate_1":
                print(f"[ELF] AxisCos={elf_axis_angle:.3f}, Aniso={elf_anisotropy:.3f}")
        ...
        # === 新增：6.51 vs 6.52 偏向性特征（路线 A 核心） ===
        delta_sum  = ml_6_51[0] - ml_6_52[0]
        delta_max  = ml_6_51[1] - ml_6_52[1]
        delta_norm = ml_6_51[2] - ml_6_52[2]
        ...
        global_feats = np.concatenate([
            ...
            ml_6_51,
            ml_6_52,
            [delta_sum, delta_max, delta_norm],
            [orientation_feat],
            [elf_axis_angle],
            [elf_anisotropy]
        ])
        ...
        # 最终特征向量: 54 + 11 = 65维
        feat = np.concatenate([flat_atoms, global_feats])  # 65维
