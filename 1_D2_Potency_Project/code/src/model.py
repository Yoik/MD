...
class EfficiencyPredictor(nn.Module):
    def __init__(self, input_dim = 133, dropout_rate=0.2):
        ...
        self.n_atoms = config.get_int("data.n_atoms")
        self.atom_feat_dim = config.get_int("data.atom_feat_dim")
        self.global_feat_dim = config.get_int("data.global_feat_dim")

        self.atom_hidden_dim = config.get_int("model.atom_hidden_dim", default=64)
    ...
    def forward_one(self, x):
        expected_dim = self.n_atoms * self.atom_feat_dim + self.global_feat_dim
        assert x.shape[-1] == expected_dim, \
            f"Input dim mismatch: got {x.shape[-1]}, expected {expected_dim}"

        # x input shape: [Batch, Frames, n_atoms*atom_feat_dim + global_feat_dim]
