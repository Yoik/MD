...
# ==============================================================================
# 自定义 Dataset：用于 Ranking 任务
# ==============================================================================
class RankingDataset(Dataset):
    def __init__(self, query_feats, query_labels, query_ids,
                 ref_feats_by_pose, ref_value=1.0):
        """
        :param query_feats: 待测化合物的切片列表
        :param ref_feats_by_pose: 参考化合物（已切片、已归一化）按 pose 分组的字典
        :param ref_value: 参考化合物的效能值 (通常为 1.0 或 100.0)
        """
        self.query_feats = query_feats
        self.query_labels = query_labels
        self.query_ids = query_ids
        self.ref_feats_by_pose = ref_feats_by_pose  # 保存所有参考片段供随机采样
        ...
        for pose_id, feats_list in ref_feats_by_pose.items():
            if len(feats_list) == 0:
                continue
            self.ref_pose_ids.append(pose_id)
            self.ref_tensors.append(
                torch.from_numpy(np.stack(feats_list)).float()
            )

        if len(self.ref_tensors) == 0:
            raise ValueError("Reference pose tensors are empty!")
    ...
    def __getitem__(self, idx):
        ...
        if q_score >= self.ref_value - 0.05: # 允许 5% 容差
            rank_label = 1.0
        else:
            rank_label = -1.0
...
def get_pose_id_from_ref_path(ref_file_path):
    """
    从 *_features.npy 的路径中提取 pose ID
    例如:
    .../charmm-gui-6321110432_gromacs_replicate_1/..._features.npy
    -> charmm-gui-6321110432
    """
    parent = os.path.basename(os.path.dirname(ref_file_path))
    return parent.split("_gromacs")[0]
...
def prepare_data(label_file, result_dir, ref_feature_path,  # <--- [新增参数]
    ...
    # === 新：按 pose 分组 reference slices ===
    ref_slices_by_pose = defaultdict(list)
    total_ref_frames = 0

    for ref_file, ref_data in zip(ref_files, ref_raw_list):
        pose_id = get_pose_id_from_ref_path(ref_file)
        ...
        if n_frames > window_size:
            for start in range(0, n_frames - window_size + 1, stride):
                ...
                ref_slices_by_pose[pose_id].append(slice_data)
        else:
            if n_frames > 0:
                ref_slices_by_pose[pose_id].append(ref_data)

    print(f"  -> Loaded {total_ref_frames} frames from reference.")
    for pose_id, slices in ref_slices_by_pose.items():
        print(f"     Pose {pose_id}: {len(slices)} reference slices")
    ...
    # 注意：必须使用同一个 scaler！
    ref_processed_by_pose = {}

    for pose_id, slices in ref_slices_by_pose.items():
        ref_processed_by_pose[pose_id] = process_features_list(
            slices, scaler, dist_indices
        )
    ...
    train_dataset = RankingDataset(
        ...
        ref_feats_by_pose=ref_processed_by_pose,  # [新增]
    ...
    test_dataset = RankingDataset(
        ...
        ref_feats_by_pose=ref_processed_by_pose,  # [新增] 测试集也用同样的 Reference
        ref_value=1.0
    )
