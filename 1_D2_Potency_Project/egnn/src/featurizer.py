...
class PhysicsFeaturizer:
    ...
    def _get_atom_feature(self, atom, electronic_weight=0.0, is_ligand=False, resid_onehot=None, clash_score=0.0):
            ...
            return norm_props + phys_feat + clash_feat + res_feat
    
    def process_frame(self, u, lig_res, raw_weights, md_ring_indices):
        # 1. 动态获取残基 ID
        ...
        for i, atom in enumerate(lig_heavy):
            ...
            # [新增] 获取对应的 clash score
            c_score = clash_scores[i]
            
            # [修改] 传入 clash_score
            node_feats.append(self._get_atom_feature(atom, w, is_ligand=True, clash_score=c_score))
        ...
        for atom in rec_heavy:
            ...
            # [修改] 受体原子 clash_score 设为 0.0
            node_feats.append(self._get_atom_feature(atom, 0.0, is_ligand=False, resid_onehot=onehot, clash_score=0.0))
