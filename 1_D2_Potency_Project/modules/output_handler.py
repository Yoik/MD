...
def calculate_interaction_strength(elf_weights, angles_389, angles_390, 
    ...
    # 归一化强度 (0-1)
    strength_389 = np.sum(combined_389) / (6.0 * np.max(elf_weights) + 1e-10)  # 相对于最强可能值
    strength_390 = np.sum(combined_390) / (6.0 * np.max(elf_weights) + 1e-10)
...
def format_interaction_strength(strength_dict):
    ...
    lines = [
        ...
        "",
        f"Phe389 主要贡献碳: C{strength_dict['major_contributor_389']}",
        f"  平均夹角: {strength_dict['avg_angle_389']:.1f}° (标准差: {strength_dict['std_angle_389']:.1f}°)",
        "",
        f"Phe390 主要贡献碳: C{strength_dict['major_contributor_390']}",
